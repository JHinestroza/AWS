...
func ValidarDatosMKDIR(context []string, particion Structs.Particion, pth string) string {
	path := ""
	p := false
	...
	if path == "" {
		return Error("MKDIR", "Se necesitan parametros obligatorio para crear un directorio.")

	}
	tmp := GetPath(path)
	return mkdir(tmp, p, particion, pth)
}

func mkdir(path []string, p bool, particion Structs.Particion, pth string) string {
	copia := path
	...
	if err != nil {
		return Error("MKDIR", "No se ha encontrado el disco.")
	...
	if err_ != nil {
		return Error("MKDIR", "Error al leer el archivo")
	...
	var newf string
	if len(path) == 0 {
		return Error("MKDIR", "No se ha brindado una path válida")
	...
	for v := 0; v < len(path)-1; v++ {
					...
					for j := 0; j < 4; j++ {
						...
						if Comparar(nombreCarpeta, path[v]) {
							...
							if inode.I_uid != int64(Logged.Uid) {
								return Error("MKDIR", "No tiene permisos para crear carpetas en este directorio.")
		...
		if !fnd {
			if p {
				...
				return Error("MKDIR", "No se pudo crear el directorio: "+direccion+", no existen directorios.")
			...
			if i < 16 {
				...
				for j := 0; j < 4; j++ {
					...
					if folder.B_content[j].B_inodo == -1 {
						...
						if bi == -1 {
							return Error("MKDIR", "No se ha podido crear el directorio")
	...
	file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*bb+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*bb, 0) //TODO AGREGARLE LOS 16 ARCHIVOS
	...
	file.Close()
	return Mensaje("MKDIR", "Se ha creado el directorio "+ruta)
}
...
func updatebm(spr Structs.SuperBloque, pth string, t string) string {
	ch := 'x'
	var num int
	//file, err := os.OpenFile(strings.ReplaceAll(pth, "\"", ""), os.O_WRONLY, os.ModeAppend)
	file, err := os.Open(strings.ReplaceAll(pth, "\"", ""))
