...
func mkfile(path []string, p bool, particion Structs.Particion, pth string) {
		...
		for i := 0; i < 16; i++ {
			if i < 16 {
				if inode.I_block[i] != -1 {
					folder = Structs.NewBloquesCarpetas()
					file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*inode.I_block[i]+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*inode.I_block[i], 0)
					...
					for j := 0; j < 4; j++ {
						...
						if Comparar(nombreCarpeta, path[v]) {
							...
							inode = Structs.NewInodo()
							file.Seek(spr.S_inode_start+int64(unsafe.Sizeof(Structs.Inodo{}))*folder.B_content[j].B_inodo, 0)
			...
			if i < 16 {
				...
				for j := 0; j < 4; j++ {
					...
					if folder.B_content[j].B_inodo == -1 {
						...
						inodetmp.I_size = int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))
		...
		for i := 0; i < 16; i++ {
			if inode.I_block[i] == -1 {
				if i < 16 {
					...
					file.Seek(spr.S_inode_start+int64(unsafe.Sizeof(Structs.Inodo{}))*father, 0)
	...
	file.Seek(spr.S_inode_start+int64(unsafe.Sizeof(Structs.Inodo{}))*bi, 0)
	var binInodeTemp bytes.Buffer
	binary.Write(&binInodeTemp, binary.BigEndian, inodetmp)
	EscribirBytes(file, binInodeTemp.Bytes())

	file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*bb+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*bb, 0)
	var binFolderTmp bytes.Buffer
	binary.Write(&binFolderTmp, binary.BigEndian, foldertmp)
	EscribirBytes(file, binFolderTmp.Bytes())

	file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*past+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*past, 0)
					...
					for j := 0; j < 4; j++ {
						...
						if Comparar(nombreFIle, path[v]) {
							...
							if inode.I_type == 1 && nombreFIle == path[len(path)-1] {
								if j == 2 {
									archivo := Structs.BloqueArchivos{}
									contenido := ""
									for k := 0; k < 16; k++ {
										file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*inodeAux.I_block[i]+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*inodeAux.I_block[i]+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*int64(k), 0)
									...
									contenido := ""
									for k := 0; k < 16; k++ {
										file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*inodeAux.I_block[i]+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*inodeAux.I_block[i]+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*int64(16+k), 0)
										if inode.I_block[k] == -1 {
											break
										}
										data = leerBytes(file, int(unsafe.Sizeof(Structs.BloqueArchivos{})))
					...
					for j := 0; j < 4; j++ {
						...
						if Comparar(nombreFIle, path[v]) {
							...
							if inode.I_type == 1 && path[v] == path[len(path)-1] {
								archivo := Structs.BloqueArchivos{}
								file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*inode.I_block[i]+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*inode.I_block[i]+int64(unsafe.Sizeof(Structs.BloqueCarpetas{})), 0)
								...
								file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*inodeAux.I_block[i]+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*inodeAux.I_block[i], 0)
