...
func dks(p string, id string) string {
	...
	positions[0] = disk.MBR_particion1.Part_start - (1 + int64(unsafe.Sizeof(Structs.MBR{})))
	positions[1] = disk.MBR_particion2.Part_start - disk.MBR_particion1.Part_start + disk.MBR_particion1.Part_size
	positions[2] = disk.MBR_particion3.Part_start - disk.MBR_particion2.Part_start + disk.MBR_particion2.Part_size
	positions[3] = disk.MBR_particion4.Part_start - disk.MBR_particion3.Part_start + disk.MBR_particion3.Part_size
	positions[4] = disk.MBR_tamanio + 1 - disk.MBR_particion4.Part_start + disk.MBR_particion4.Part_size
	...
	if ext {
		...
		for auxEbr.Part_next != -1 {
			...
			res := float64(auxEbr.Part_size) / float64(disk.MBR_tamanio)
			res = res * 100
			tmplogic += "<td>\"EBR\"</td>"
			...
			resta = resta / float64(disk.MBR_tamanio)
	...
	for i := 0; i < 4; i++ {
		if partitions[i].Part_type == 'E' {
			...
			res := float64(partitions[i].Part_size) / float64(disk.MBR_tamanio)
	...
	if tamPrim != 0 {
		libre := disk.MBR_tamanio - tamPrim
		res := float64(libre) / float64(disk.MBR_tamanio)
...
func tree(p string, id string) string {
	var pth string
	spr := Structs.NewSuperBloque()
	inode := Structs.NewInodo()
	partition, mensaje := GetMount("REP", id, &pth)
	if mensaje != "" {
		return mensaje
	}

	if partition.Part_start == -1 {
		return Error("REP", "No se ha creado el reporte.")
	}
	...
	for i := 0; i < int(freeI); i++ {
			...
			for j := 0; j < 16; j++ {
				if inode.I_block[j] != -1 {
					...
					if existeEnArreglo(bloquesUsados, inode.I_block[j]) == 1 {
						foldertmp := Structs.BloqueCarpetas{}

						file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*inode.I_block[j]+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*inode.I_block[j], 0)
						data = leerBytes(file, int(unsafe.Sizeof(Structs.BloqueCarpetas{})))
						buffer = bytes.NewBuffer(data)
				...
				if inode.I_block[j] != -1 {
					if j < 16 {
						...
						for {
							...
							file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*contador+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*contador, 0)
							data = leerBytes(file, int(unsafe.Sizeof(Structs.BloqueCarpetas{})))
							buffer = bytes.NewBuffer(data)
							err_ = binary.Read(buffer, binary.BigEndian, &foldertmp)
						...
						if posicion == 2 || posicion == 0 {
							file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*contador+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*contador+int64(unsafe.Sizeof(Structs.BloqueCarpetas{})), 0)
							for k := 0; k < 16; k++ {
								contadorArchivos++
								...
								filetmp := Structs.BloqueArchivos{}
							...
							file.Seek(spr.S_block_start+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))*contador+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*32*contador+int64(unsafe.Sizeof(Structs.BloqueCarpetas{}))+int64(unsafe.Sizeof(Structs.BloqueArchivos{}))*int64(16), 0)
							for k := 0; k < 16; k++ {
								contadorArchivos++
								if inode.I_block[k] == -1 {
									break
		...
		file.Seek(spr.S_inode_start+int64(unsafe.Sizeof(Structs.Inodo{}))*int64(i+1), 0)
		data = leerBytes(file, int(unsafe.Sizeof(Structs.Inodo{})))
		buffer = bytes.NewBuffer(data)
		err_ = binary.Read(buffer, binary.BigEndian, &inode)
		if err_ != nil {
...
func fileR(p string, id string, ruta string) string {
	...
	var path string
	particion, mensaje := GetMount("MKDIR", id, &path)
	if mensaje != "" {
		return mensaje
	}
	tmp := GetPath(ruta)
